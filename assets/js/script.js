...
const projectModalFunc = function () {
  ...
  if (projectModalOverlay) projectModalOverlay.classList.toggle("active");
}
...
if (projectItems.length && projectModalContainer) {
  projectItems.forEach(function (item) {
    item.addEventListener("click", function (event) {
      ...
      const status = projectCard.querySelector(".project-status")?.textContent || "";
      const description = projectItem?.dataset.description || "";
      const date = projectItem?.dataset.date || projectCard.querySelector(".project-date")?.textContent || "";
      const repoUrl = projectItem?.dataset.repo || "";
      const tags = projectItem?.dataset.tags ? projectItem.dataset.tags.split(",") : [];

      projectModalTitle.innerText = title;
      projectModalStatus.innerText = status;
      projectModalDescription.innerText = description;
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const defaultPage = currentNav ? currentNav.innerHTML.toLowerCase() : "about";
  const currentPage = hashPage || defaultPage;

  for (let i = 0; i < pages.length; i++) {
    if (pages[i].dataset.page === currentPage) {
      pages[i].classList.add("active");
  ...
  if (hashPage) {
    const target = document.getElementById(hashPage);
